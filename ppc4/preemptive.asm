...
00149$:
	...
	rlc	a
	mov	_clockwise,a
;	preemptive.c:105: if(currentThread == 3) currentThread = 2;
	mov	a,#0x03
	cjne	a,_currentThread,00102$
	mov	_currentThread,#0x02
	sjmp	00109$
00102$:
;	preemptive.c:106: else currentThread++;
	mov	a,_currentThread
	inc	a
	mov	_currentThread,a
	sjmp	00109$
00108$:
;	preemptive.c:109: clockwise = (currentThread == 0);
	clr	a
...
00153$:
	mov	_clockwise,a
;	preemptive.c:110: if(currentThread == 0) currentThread = 1;
	mov	a,_currentThread
	jnz	00105$
	mov	_currentThread,#0x01
	sjmp	00109$
00105$:
;	preemptive.c:111: else currentThread--;
	mov	a,_currentThread
	dec	a
	mov	_currentThread,a
00109$:
;	preemptive.c:113: temp = 1 << currentThread;
...
00156$:
	djnz	b,00155$
	mov	_temp,a
;	preemptive.c:114: if (threadMask & temp){
	mov	a,_temp
	anl	a,_threadMask
	jz	00112$
;	preemptive.c:118: RESTORESTATE;
	mov	a,_currentThread
	add	a, #_savedSP
	mov	r1,a
	mov	_SP,@r1
...
;------------------------------------------------------------
;	preemptive.c:127: ThreadID ThreadCreate(FunctionPtr fp){
;	-----------------------------------------
;	 function ThreadCreate
;	-----------------------------------------
_ThreadCreate:
;	preemptive.c:133: EA = 0;
;	assignBit
	clr	_EA
;	preemptive.c:134: if(threadMask == 0x0F){
	mov	a,#0x0f
...
;	preemptive.c:135: return -1;
	...
	ret
00102$:
;	preemptive.c:176: for(i = 0; i != MAXTHREADS; i++){
	mov	_i,#0x00
00107$:
	mov	a,#0x04
	cjne	a,_i,00137$
	sjmp	00105$
00137$:
;	preemptive.c:178: temp = 1;
	mov	_temp,#0x01
;	preemptive.c:179: temp <<= i;
	mov	b,_i
	inc	b
	mov	a,_temp
	sjmp	00139$
...
00139$:
	djnz	b,00138$
	mov	_temp,a
;	preemptive.c:180: if(!(threadMask & temp)){
	mov	a,_temp
	anl	a,_threadMask
	jnz	00108$
;	preemptive.c:182: threadMask |= temp;
	mov	a,_temp
	orl	_threadMask,a
;	preemptive.c:183: newThread = i;
	mov	_newThread,_i
...
;	preemptive.c:176: for(i = 0; i != MAXTHREADS; i++){
	...
	sjmp	00107$
00105$:
;	preemptive.c:188: tempSP = SP;
	mov	_tempSP,_SP
;	preemptive.c:191: SP = (0x3F) + newThread * (0x10);
	mov	a,_newThread
	swap	a
	anl	a,#0xf0
	mov	r7,a
	add	a,#0x3f
	mov	_SP,a
;	preemptive.c:196: __endasm;
	PUSH	DPL
	PUSH	DPH
;	preemptive.c:204: __endasm;
	ANL	A, #0
	PUSH	ACC
	PUSH	ACC
	PUSH	ACC
	PUSH	ACC
;	preemptive.c:206: PSW = (newThread << 3);
	mov	a,_newThread
	mov	r7,a
	swap	a
	rr	a
	anl	a,#0xf8
	mov	_PSW,a
;	preemptive.c:209: __endasm;
	PUSH	PSW
;	preemptive.c:211: savedSP[newThread] = SP;
	mov	a,_newThread
	add	a, #_savedSP
	mov	r0,a
	mov	@r0,_SP
;	preemptive.c:213: SP = tempSP;
	mov	_SP,_tempSP
;	preemptive.c:215: EA = 1;
;	assignBit
	setb	_EA
;	preemptive.c:216: return newThread;
	mov	dpl, _newThread
;	preemptive.c:217: }
	ret
;------------------------------------------------------------
...
;	preemptive.c:229: do{
00103$:
;	preemptive.c:240: currentThread = (currentThread < 3) ? currentThread + 1 : 0;
	mov	a,#0x100 - 0x03
...
;	preemptive.c:241: temp = 1 << currentThread;
	mov	b,_currentThread
	inc	b
	mov	a,#0x01
	sjmp	00130$
...
00130$:
	djnz	b,00129$
	mov	_temp,a
;	preemptive.c:242: if (threadMask & temp){
	mov	a,_temp
	anl	a,_threadMask
	jz	00103$
...
;------------------------------------------------------------
;	preemptive.c:255: void ThreadExit(void)
;	-----------------------------------------
;	 function ThreadExit
;	-----------------------------------------
_ThreadExit:
;	preemptive.c:263: EA = 0;
;	assignBit
	clr	_EA
;	preemptive.c:264: temp = 1 << currentThread;
	mov	b,_currentThread
	inc	b
	mov	a,#0x01
	sjmp	00136$
...
00136$:
	djnz	b,00135$
	mov	_temp,a
;	preemptive.c:265: threadMask ^= temp;
	mov	a,_temp
	xrl	_threadMask,a
...
00139$:
	djnz	b,00138$
	mov	_temp,a
;	preemptive.c:268: if(temp & threadMask){
	mov	a,_threadMask
	anl	a,_temp
	jz	00108$
;	preemptive.c:269: currentThread = i;
	mov	_currentThread,_i
;	preemptive.c:270: break;
	sjmp	00103$
00108$:
;	preemptive.c:266: for(i = 0; i < MAXTHREADS; i++){
	mov	a,_i
	...
	sjmp	00107$
00103$:
;	preemptive.c:273: if (i == MAXTHREADS){
	mov	a,#0x04
	cjne	a,_i,00105$
;	preemptive.c:274: currentThread = -1;
